...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const totalTriggers = formattedRules.reduce(
      (sum, r) => sum + r.triggeredCount,
      0,
    );
    ...
    const mostTriggeredRule =
      formattedRules.sort((a, b) => b.triggeredCount - a.triggeredCount)[0]
        ?.name || null;
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { name, ruleType, description, priority, action, conditions } =
      await request.json();
    ...
    return NextResponse.json({
      ...
      rule: {
        ...
        conditions:
          typeof newRule.conditions === "string"
