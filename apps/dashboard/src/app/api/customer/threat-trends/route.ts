  ...
  try {
    ...
    const threatTypeDistribution = await database.queryMany(
      `SELECT 
         ...
         AVG(CASE WHEN severity = 'CRITICAL' THEN 9 WHEN severity = 'HIGH' THEN 7 WHEN severity = 'MEDIUM' THEN 5 ELSE 3 END) as avg_risk_score
       FROM threat_events 
       WHERE tenant_id = $1 AND created_at >= NOW() - INTERVAL '${interval}'
    ...
    const recentHighThreats = await database.queryMany(
      `SELECT 
         id, threat_type, severity, source_ip, 'unknown' as target_endpoint, 
         CASE WHEN severity = 'CRITICAL' THEN 9 WHEN severity = 'HIGH' THEN 7 WHEN severity = 'MEDIUM' THEN 5 ELSE 3 END as risk_score, status, created_at, 'Threat detected' as details
       FROM threat_events 
    ...
    return NextResponse.json({
      ...
      threatTrends: {
        ...
        summary: {
          ...
          uniqueThreatTypes: parseInt(
            threatSummary?.unique_threat_types || "0",
          ),
          ...
          blockRate:
            totalThreats > 0
              ? Math.round((blockedThreats / totalThreats) * 100 * 100) / 100
              : 0,
        },
    ...
    logger.error("Threat trends API error:", {
