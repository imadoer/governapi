...
export async function runRealScan(scanId: string, target: string, scanType: string) {
  try {
    await supabase
      .from('scans')
    ...
    return new Promise((resolve, reject) => {
      const child = spawn('node', [compiledScanner, target], {
        ...
        env: {
          ...process.env,
          DATABASE_URL: 'postgresql://scanner_user:scanner_pass_123@localhost:5432/governapi_scanner',
          TENANT_ID: scanId
        }
      })
